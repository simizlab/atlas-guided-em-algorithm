...
int main(int argc, char **argv) {
	...
	if (argc != 7 && argc != 8) {
		...
		std::cerr << argv[0] << " <input feature directory> <input abdominal cavity mask directory> "
			<< " <input param directory> <output directory> <test filename list> <feature name list> "
	...
	const std::string test_filename_list_path = argv[5];
	const std::string feature_name_list_path = argv[6];
	std::string input_atlas_dir;
	if (argc == 8) {
		input_atlas_dir = std::string(argv[7]) + "/";
	}
	...
	std::cout << "----- Load init param -----" << std::endl;
	std::vector<Eigen::MatrixXd> mean_init(TESTTIME_NUM_LABELS, Eigen::MatrixXd(num_features, 1));
	std::vector<Eigen::MatrixXd> covariance_init(TESTTIME_NUM_LABELS, Eigen::MatrixXd(num_features, num_features));
	for (int l = 0; l < TESTTIME_NUM_LABELS; l++) {
		read_raw_to_eigen(mean_init[l], input_param_dir + TESTTIME_LABEL_NAMES[l] + "/mean_param.raw", num_features);
		read_raw_to_eigen(covariance_init[l], input_param_dir + TESTTIME_LABEL_NAMES[l] + "/covariance_param.raw", num_features);
	}
	//for (int l = 0; l < TESTTIME_NUM_LABELS; l++) {
	//	std::cout << mean_init[l] << std::endl;
	//}
	...
	for (auto case_itr = test_filename_list.begin(); case_itr != test_filename_list.end(); ++case_itr) {
		...
		std::cout << "----- Start EM algorithm -----" << std::endl;
		std::vector<Eigen::MatrixXd> mean(mean_init);
		std::vector<Eigen::MatrixXd> covariance(covariance_init);
		size_t iteration_counter;
		em_algorithm::estimate_mixture_using_atlas(feature_array,
													mean,
													covariance,
													atlas_array,
													num_pixel,
		...
		std::cout << "----- Save estimated param -----" << std::endl;
		for (int l = 0; l < TESTTIME_NUM_LABELS; l++) {
